...
!(function ($) {
  ...
  function extractFormVal(input) {
    ...
    if (input && input.length) {
      ...
      if (res && res.length) {
        ...
        dataToSend['blood_group'] = $('#rBlood').val();
        dataToSend['relation'] = $('#relation').val();
        dataToSend['gender'] = $('input[name="rgender"]:checked').val();
        dataToSend['on_date'] = currentDate.format('YYYY-MM-DD');
        dataToSend['city_hospital'] = $('#rcity').val();
      }
  ...
  $('form.php-email-form').submit(function (e) {
    ...
    if ($('#relation').val() ===
        null) {
        $('.validateSelR').html('Relation is a mandatory field');
        $('.validateSelR').css('display', 'block');
        ferror = true;
    }
    else if ($('#relation').val().length) {
        $('.validateSelR').css('display', 'none');
    }
    if ($('#rcity').val() ===
      null) {
    ...
    if ($(this).data('recaptcha-site-key')) {
      ...
      if ($('input[name="condition"]:checked').val() === 'on') {
  ...
  function php_email_form_submit(this_form, action, data, data1) {
    $.ajax({
      ...
      if (msg.status == 200) {
        sessionStorage.setItem('city', data1.city_hospital);
        sessionStorage.setItem('bg', data1.blood_group);
        this_form.find('.loading').slideUp();
        ...
        $('#relation').val('');
        $('#rcity').val('');
        setTimeout(function () { window.location.href = 'donors_list.html'; }, 1000);
